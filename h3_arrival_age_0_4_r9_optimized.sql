...
source_areas AS (
  ...
  CROSS JOIN _mapp_h3_scope AS scope
  WHERE oa.geom IS NOT NULL
    AND oa.ts018_0004 IS NOT NULL
    AND oa.ts018_0001 IS NOT NULL
    AND public.ST_Intersects(oa.geom, scope.geom_4326)
),
scope_cells AS (
  SELECT h3_polygon_to_cells(scope.geom_4326, 9) AS h3_id
  FROM _mapp_h3_scope AS scope
),
cells AS (
  SELECT
    scope_cells.h3_id,
...
weighted AS (
  SELECT
    overlap.h3_id,
    overlap.geom_4326,
    SUM(overlap.ts018_0004 * overlap.overlap_m2 / NULLIF(overlap.area_m2, 0.0))::double precision AS arrived_age_0_4,
    SUM(overlap.ts018_0001 * overlap.overlap_m2 / NULLIF(overlap.area_m2, 0.0))::double precision AS total_population
